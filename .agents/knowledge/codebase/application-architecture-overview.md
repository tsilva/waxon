...
* Low-scoring review answers remain due immediately: `scheduleNextReview` returns the current time below the scheduled-score threshold, and resolved evaluation status should preserve that persisted `nextDue` so question stats can show "Now" rather than "Unknown".
