...
* Static shell headers reserve the right-side toolbar slot with `reader-actions-placeholder`; `PersistentReviewToolbarActions` must stay mounted inside `AuthenticatedProviders` so the due count and signed-in user menu/avatar appear after client auth hydration.
