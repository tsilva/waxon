...
* Learn chat answer submissions use one combined `course_answer_decision` LLM call to both grade/record the answered question and decide milestone progress before the next tutor stream. The route still enforces `requireCourseMilestoneMastery()` locally, so milestone advancement requires a recorded high-scoring answer. The course chat SSE `done` payload includes `latencyMetrics` with `answer_decision_ms`, `time_to_first_delta_ms`, and `chat_stream_ms`.
