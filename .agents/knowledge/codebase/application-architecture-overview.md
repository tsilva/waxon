...
* Learn course intake, TOC generation, and tutor turns use the Learn-specific OpenRouter model config (`LLM_LEARN_MODEL`, default `google/gemini-2.5-flash`) instead of the generic `LLM_MODEL`, so global chat-model overrides do not leak into the learner-facing course path.
