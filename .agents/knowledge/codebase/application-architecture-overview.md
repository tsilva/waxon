...
* Learn course intake, TOC generation, and tutor turns use the Learn-specific OpenRouter model config (`LLM_LEARN_MODEL`, default `anthropic/claude-haiku-4.5`) instead of the generic `LLM_MODEL`, so global chat-model overrides do not leak into the learner-facing course path.
* Learn chat answer submissions first try a single streaming `course_chat_turn` path on the Learn model (`LLM_LEARN_MODEL`) with both `record_course_answer_decision` and `render_question_widget` tools. The route buffers a valid streamed answer decision, then persists it and applies the LLM's well-formed progress decision mechanically after the same stream also proves it has visible tutor text and a widget; `requireCourseMilestoneMastery()` is not used in this new path. If the stream omits a valid decision, visible tutor text, or a widget, `/api/courses/chat` emits `rollback`, retries once from the learner-answer checkpoint, then falls back to the older two-call path using the same Learn model for both answer decision and tutor continuation.
* The course chat SSE `done` payload includes `latencyMetrics` with `answer_decision_ms`, `time_to_first_delta_ms`, `chat_stream_ms`, and optional `rollback_count`. Course message metrics also persist OpenRouter prompt-cache usage when available: cached prompt tokens, uncached prompt tokens, cache-write tokens, and cache-hit percentage.
* Learn LLM prompts should keep immutable instructions as the request prefix and put dynamic topic, course, milestone, progress, and conversation fields later. For example, course TOC generation keeps the reusable TOC instructions before the dynamic `Topic:` line so provider prompt caching can reuse the largest stable prefix. OpenRouter prompt-cache hits require explicit `cache_control` breakpoints in message content and provider/model support verified with real usage metrics. The Learn tutor turn uses one cacheable immutable tutor-instruction block, keeps course/milestone/progress/history after the breakpoint, and uses a stable per-user Learn chat `session_id` with a prompt-shape version suffix. On 2026-06-24, real Learn turns required a roughly 21k-character immutable tutor block on `anthropic/claude-haiku-4.5` before OpenRouter reported cache writes and repeat cache reads.
