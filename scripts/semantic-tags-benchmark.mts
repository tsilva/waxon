...
const relatedQuestionsSql = `
  ...
  SELECT question_embedding.question_id,
   ...
   ORDER BY distance, question_embedding.question_id
   LIMIT 50`;
    ...
    CROSS JOIN LATERAL (
      SELECT embedding.tag_id,
       ...
       WHERE embedding.user_id = $1
         AND embedding.space_id = $2
         AND tag.deleted_at IS NULL
       ORDER BY distance, embedding.tag_id
       LIMIT 10
    ) nearest
...
try {
  ...
  const relatedQuestionsP95 = percentile(questionTimes, 0.95);
  const relatedTagsP95 = percentile(tagTimes, 0.95);
