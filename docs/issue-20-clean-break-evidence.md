...
The native attempts on `892fd84e7e97e3f462844228b069f8727186a6eb`, `bd60112447e7570908e22824271fbcf3a2af7d8f`, and `503e8a083cdb307e7592273d08da78ab5047cb4b` are superseded and provide no final acceptance evidence. The first two attempts reached the normal model path instead of retaining request-time acceptance authorization. On `503e8a0`, Answer submission failed before persistence with `Cannot read properties of undefined (reading 'id')`: the retained Turbopack server predated the `browserAcceptanceTestLearner` export rename and exposed that imported const as undefined in the app-route module graph. The repaired candidate places the shared acceptance identity in a dependency-free leaf module, retains the guarded authorization in the persisted job, and revalidates the dedicated Learner and named Prompt in the Workflow consumer. The coordinator will reset and restart all nine cases from the beginning.
...
| `pnpm test` with `APPLICATION_CONTRACT_TEST_DATABASE_URL`, `QUESTION_SEARCH_TEST_DATABASE_URL`, and both runtime DB URLs set to the disposable database | passed: 87 tests, 0 failed, 0 skipped |
