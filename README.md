...
answer evaluation to `google/gemini-3.5-flash`. The app also accepts
...
This keeps `/review`, `/admin`, `/learn`, `/library`, `/stats`, and app
...
- If the user's knowledge base has no questions, the app bootstraps it from `data/questions.csv`.
- Login and signup use Clerk. Waxon stores its own internal users and maps Clerk
  accounts through `auth_accounts`.
- The `questions` table stores per-card state directly under `user_id`.
- The `question_attempts` table stores every resolved user attempt with raw answer, concise LLM answer summary, score, justification, and timestamps.
