...
- Learner Flag dialogs use Question terminology in visible reasons, including `Question is unclear`, while preserving stable machine-readable reason identifiers.
...
- Desktop and 390 × 844 native Browser verification confirmed expanded Review feedback reveals the `Copy Markdown` action, successful copying places the complete structured handoff on the system clipboard, and the action remains contained without horizontal overflow.
...
- August 28, 2026 native Browser checks confirmed the empty Review shell remains horizontally contained at 1280 × 900 and 390 × 844 with no console warnings or errors after the Recall Result change. The local browser had no configured database, so populated feedback-row and correction interactions remain covered by automated contracts rather than this visual pass.
- August 29, 2026 native Browser checks confirmed the A/B/C score-badge change leaves the empty Review shell horizontally contained at desktop and 390 × 844 with no console warnings or errors. The local database was unavailable, so the exact Correct → A, Partial → B, and Incorrect → C badge mapping remains covered by the focused UI contract.
