...
function risk(candidate: PlanCandidate, retention: number, now: Date): number {
  ...
  return (
    candidate.importance * (shortfall * 10 + Math.min(3, overdueDays / 7)) +
    (candidate.hasGap ? 1.5 : 0)
  );
}
...
export function buildReviewPlan(input: {
  ...
  const due = input.candidates
    .filter(
      (candidate) =>
        candidate.lifecycle === "learning" ||
        candidate.lifecycle === "review",
    )
    .filter(
      (candidate) =>
        candidate.dueAt !== null && candidate.dueAt.getTime() <= horizon.getTime(),
    )
    .sort((left, right) => {
      const riskDifference =
        risk(right, input.desiredRetention, now) -
        risk(left, input.desiredRetention, now);
      if (riskDifference !== 0) {
        return riskDifference;
      }
      const dueDifference =
        (left.dueAt?.getTime() ?? 0) - (right.dueAt?.getTime() ?? 0);
      return dueDifference || left.questionId.localeCompare(right.questionId);
    });
  ...
  function admit(candidate: PlanCandidate): boolean {
    ...
    if (remainingBaseSlots <= 0 || reservedSeconds > remainingSeconds) {
      return false;
    }
  ...
  for (const candidate of waiting) {
    if (admittedNew >= input.newItemsPerDay) {
      break;
    }
    if (admit(candidate)) {
      admittedNew += 1;
